...
function calculerBesoins(spec) {
  ...
  // Duty cycle (taux de marche 0-1) et arrêt pompe entre cycles
  const dutyCycle = spec.systeme?.dutyCycle ?? 1;
  const pumpStops = spec.systeme?.pumpStops ?? false;

  // Modèle de chaleur — référentiel Jof : Pth = Pabs × ratio de pertes (selon type de circuit).
  // Ratios = borne haute des fourchettes constructeur (conservateur pour le dimensionnement froid).
  const RATIOS_PERTES = { ouvert: 0.30, ferme: 0.20, laminage: 0.50 };
  const typeCircuit = spec.systeme?.typeCircuit ?? 'ouvert';
  const ratioPertes = RATIOS_PERTES[typeCircuit] ?? RATIOS_PERTES.ouvert;
  const chaleurActive = puissanceAbsorbee * ratioPertes;        // phases de travail
  ...
  if (holdRatio > 0) {
    ...
    chaleurGeneree = chaleurActive;
  }
  if (pumpStops && dutyCycle < 1) chaleurGeneree *= dutyCycle;  // pompe arrêtée entre cycles

  hypotheses.push(`Chaleur = Pabs × ${(ratioPertes * 100).toFixed(0)} % (circuit ${typeCircuit}).`);
  if (spec.systeme?.typeCircuit == null)
    hypotheses.push('Type de circuit non spécifié : "ouvert" supposé (ratio le plus courant).');
  if (holdRatio > 0)
