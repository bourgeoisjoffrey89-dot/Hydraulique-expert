/**
 * besoins.js — Moteur de calcul headless (brique 1/2).
 *
 * calculerBesoins(spec) -> besoins
 *   Physique PURE : aucun DOM, aucun réseau, aucun catalogue.
 *   La sélection des composants (pompe/moteur/refroidisseur) est une brique
 *   séparée : selectionnerComposants(besoins, catalogue). [architecture B]
 *
 * Règles métier verrouillées :
 *   - Débit total = Σ(débits des vérins simultanés) + MAX(débits des vérins alternés).
 *   - Vérin 'existant' : bore/rod -> force.  Vérin 'nouveau' : force -> recommendedBore,
 *     puis convergence vers le même flux que l'existant.
 *   - Maintien de pression : ACCEPTÉ dans le spec, mais sa thermo (laminage pompe
 *     variable) n'est PAS modélisée en v1 -> signalée dans besoins.hypotheses,
 *     jamais inventée.
 *
 * Double usage navigateur / Node, comme formulas.js.
 */

const HF = (typeof HYDRAULIC_FORMULAS !== 'undefined')
  ? HYDRAULIC_FORMULAS
  : require('./formulas.js');

// Convertit une force exprimée en t / daN / kN / N vers des daN.
function forceEnDaN(valeur, unite) {
  switch ((unite || 'daN').toLowerCase()) {
    case 't': case 'tonnes': case 'tonne': return HF.conversion.tonnesToDaN(valeur);
    case 'kn': return valeur * 100;   // 1 kN = 100 daN
    case 'n':  return valeur / 10;    // 1 daN = 10 N
    default:   return valeur;         // déjà en daN
  }
}

// Diamètre (mm) donnant une surface cible (cm²) : aire = π(bore/10)²/4.
function boreDepuisAire(aireCm2) {
  return Math.sqrt((4 * aireCm2) / Math.PI) * 10;
}

// Débit (L/min) à partir de la 1re donnée disponible : débit direct > vitesse (m/min) > temps (s).
// Permet d'accepter les cahiers réels qui spécifient une vitesse ou un débit, pas un temps.
function resoudreDebit(debit, vitesse, aireCm2, volumeL, tempsS) {
  if (debit != null) return debit;                    // L/min fourni directement
  if (vitesse != null) return aireCm2 * vitesse / 10; // depuis une vitesse en m/min
  return HF.cylinder.flowRate(volumeL, tempsS);        // depuis le temps (comportement historique)
}

function calculerVerin(v, hypotheses) {
  const r = { nom: v.nom, type: v.type, workMode: v.workMode, quantite: v.quantite ?? 1 };
  const pression = v.pression;
  r.pression = pression;                    // pression de travail du vérin (héritée ou propre)
  r.dutyCycle = (v.dutyCycle != null) ? v.dutyCycle : null;  // taux de marche PROPRE au vérin

  // --- Résolution de la géométrie selon le type ---
  let bore = v.bore, rod = v.rod;
  if (v.type === 'nouveau') {
    const forceReqDaN = forceEnDaN(v.requiredForce, v.forceUnit);
    const aireRequise = pression > 0 ? forceReqDaN / pression : 0; // cm²
    bore = boreDepuisAire(aireRequise);
    r.recommendedBore = bore;          // théorique ; l'arrondi normalisé = étape sélection
    r.forceRequiseDaN = forceReqDaN;
    if (v.rod == null) {
      rod = null;                      // pas de tige fournie pour un vérin neuf
      hypotheses.push(`Vérin "${v.nom}" (nouveau) : tige non fournie -> côté annulaire (retour) non calculé.`);
    }
  }

  // --- Surfaces ---
  r.pistonArea = HF.cylinder.pistonArea(bore);                 // cm²
  r.annularArea = (rod != null) ? HF.cylinder.annularArea(bore, rod) : null;

  // --- Forces ---
  r.forcePush = HF.cylinder.forcePush(r.pistonArea, pression);                 // daN
  r.forcePull = (r.annularArea != null) ? HF.cylinder.forcePull(r.annularArea, pression) : null;
  r.tonnage = HF.conversion.daNToTonnes(r.forcePush);                          // t

  // --- Volumes (L) ---
  r.volumeOut = HF.cylinder.volumeOut(r.pistonArea, v.stroke);
  r.volumeIn = (r.annularArea != null) ? HF.cylinder.volumeIn(r.annularArea, v.stroke) : null;

  // --- Débits (L/min) : temps, ou VITESSE (m/min), ou DÉBIT (L/min) direct selon le cahier ---
  r.flowOut = resoudreDebit(v.debitOut, v.vitesseOut, r.pistonArea, r.volumeOut, v.timeOut);
  r.flowIn = (r.annularArea != null)
    ? resoudreDebit(v.debitIn, v.vitesseIn, r.annularArea, r.volumeIn, v.timeIn) : 0;
  r.flowMax = Math.max(r.flowOut, r.flowIn);

  // --- Vitesse sortie (cm/s) ---
  r.speedOut = HF.cylinder.speed(r.flowOut, r.pistonArea);

  // --- Maintien de pression : débit de maintien ≈ 0, chaleur intégrée au bilan thermique ---
  if (v.pressureHold === true || v.pressureHold === 'true') {
    r.pressureHold = true;
    r.holdTime = v.holdTime;
    hypotheses.push(`Vérin "${v.nom}" : maintien de pression (${v.holdTime ?? '?'} s) -> ` +
      `débit de maintien ≈ 0, chaleur de maintien intégrée au bilan thermique.`);
  }

  return r;
}

function calculerBesoins(spec) {
  const hypotheses = [];

  // Pression système : champ fourni, sinon max des pressions vérins déclarées.
  const pressionSysteme = spec.systeme?.pression
    ?? Math.max(0, ...(spec.verins || []).map(v => v.pression || 0));

  // Chaque vérin sans pression propre hérite de la pression système ("pression: null" = système).
  const verins = (spec.verins || []).map(v =>
    calculerVerin({ ...v, pression: v.pression ?? pressionSysteme }, hypotheses));

  // --- Débit total : Σ(simultanés) + MAX(alternés) ---
  let sommeSimultanes = 0;
  let maxAlterne = 0;
  for (const r of verins) {
    if (r.quantite === 0) continue;          // vérin désactivé -> exclu du débit (comme l'appli)
    if (r.workMode === 'alterné') maxAlterne = Math.max(maxAlterne, r.flowMax);
    else sommeSimultanes += r.flowMax * r.quantite;   // 'simultané' par défaut
  }
  const debitTotal = sommeSimultanes + maxAlterne;

  // --- Puissance ---
  const rendement = spec.systeme?.rendement ?? 0.9;
  const puissanceHydraulique = HF.power.hydraulic(pressionSysteme, debitTotal);
  const puissanceAbsorbee = HF.power.absorbed(puissanceHydraulique, rendement);
  const pertes = HF.power.losses(puissanceAbsorbee, puissanceHydraulique);

  // --- Réservoir (si géométrie ou volume fourni) ---
  let reservoir = null;
  const res = spec.reservoir;
  if (res) {
    if (res.volume != null) {
      reservoir = { volume: res.volume, surface: null };
      hypotheses.push('Réservoir : volume fourni directement, surface d\'échange non déduite.');
    } else if (res.hauteur && res.longueur && res.largeur) {
      const L = res.longueur / 1000, l = res.largeur / 1000, h = res.hauteur / 1000; // mm -> m
      reservoir = {
        volume: L * l * h * 1000,                       // m³ -> L
        surface: 2 * (L * l + L * h + l * h)             // m² (toutes faces)
      };
    }
  }

  // --- Thermique : modèle complet (type de pompe, maintien de pression, duty cycle) ---
  // Réplique de calculateThermal. Base = puissance absorbée THÉORIQUE. L'appli, elle, prend la
  // puissance NOMINALE du moteur choisi quand il existe -> elle surestime un peu (cf hypothèse).
  const fluide = spec.fluide || {};
  const deltaT = (fluide.tempHuileMax != null && fluide.tempAmbiante != null)
    ? fluide.tempHuileMax - fluide.tempAmbiante : null;
  const coeff = fluide.coeffConvection ?? 0.010;
  const dissipationNaturelle = (reservoir?.surface != null && deltaT != null)
    ? HF.thermal.naturalDissipation(reservoir.surface, deltaT, coeff) : null;

  // Ratio de maintien de pression (temps maintien / temps cycle) sur les vérins concernés
  let totalHold = 0, totalCycle = 0;
  for (const v of (spec.verins || [])) {
    if ((v.pressureHold === true || v.pressureHold === 'true') && v.holdTime) {
      const tO = parseFloat(v.timeOut) || 0, tI = parseFloat(v.timeIn) || 0, tH = parseFloat(v.holdTime) || 0;
      totalHold += tH;
      totalCycle += tO + tI + tH;
    }
  }
  const holdRatio = totalCycle > 0 ? totalHold / totalCycle : 0;

  // Modèle de chaleur — référentiel Jof : Pth = Pabs × ratio de pertes.
  // Deux axes ORTHOGONAUX (ne pas confondre) :
  //   - typeCircuit = topologie : 'ouvert' (30 %) ou 'ferme' (20 %).
  //   - laminage    = régulation par étranglement présente (booléen) : domine et pousse à 50 %.
  // Ratios = borne haute des fourchettes constructeur (conservateur pour le dimensionnement froid).
  const typeCircuit = (spec.systeme?.typeCircuit === 'ferme') ? 'ferme' : 'ouvert';
  const laminage = spec.systeme?.laminage === true || spec.systeme?.laminage === 'true';
  const ratioPertes = laminage ? 0.50 : (typeCircuit === 'ferme' ? 0.20 : 0.30);

  const pumpStops = spec.systeme?.pumpStops ?? false;
  const estVariable = (spec.systeme?.typePompe === 'variable');

  // Chaleur de TRAVAIL (hors maintien).
  // Si AU MOINS un vérin porte son propre dutyCycle -> on somme les contributions par vérin :
  //   Σ ( Pabs_i × ratio × duty_i × quantité_i ), chacun à SON débit, SA pression, SON taux.
  // C'est le modèle juste pour une centrale alternée : on ne multiplie pas le PIC par un taux moyen.
  // Sinon -> modèle agrégé historique (pic × ratio), pondéré plus bas par le duty système.
  const dutyParVerin = verins.some(r => r.dutyCycle != null && r.quantite !== 0);
  let chaleurActive;
  if (dutyParVerin) {
    chaleurActive = 0;
    for (const r of verins) {
      if (r.quantite === 0) continue;
      const duty = (r.dutyCycle != null) ? r.dutyCycle : 1;
      if (r.dutyCycle == null)
        hypotheses.push(`Vérin "${r.nom}" : taux de marche absent (d'autres en ont un) -> 100 % supposé (probable surestimation).`);
      const pAbsI = HF.power.absorbed(HF.power.hydraulic(r.pression, r.flowMax), rendement);
      chaleurActive += pAbsI * ratioPertes * duty * r.quantite;
    }
  } else {
    chaleurActive = puissanceAbsorbee * ratioPertes;
  }

  // Surcouche maintien (dynamique non captée par le ratio établi) :
  //  - pompe fixe     : tout passe au limiteur -> chaleur ≈ Pabs sur la phase de maintien
  //  - pompe variable : cylindrée ≈ 0 -> chaleur ≈ 0 sur la phase de maintien
  let chaleurGeneree = chaleurActive;
  if (holdRatio > 0) {
    if (dutyParVerin) {
      hypotheses.push('Maintien de pression non encore combiné au duty par-vérin -> maintien non intégré à ce calcul.');
    } else {
      const chaleurMaintien = estVariable ? 0 : puissanceAbsorbee;
      chaleurGeneree = chaleurActive * (1 - holdRatio) + chaleurMaintien * holdRatio;
    }
  }

  // Duty SYSTÈME : seulement en l'absence de duty par-vérin (sinon déjà intégré à la somme).
  const dutyCycle = spec.systeme?.dutyCycle ?? 1;
  if (!dutyParVerin && pumpStops && dutyCycle < 1) chaleurGeneree *= dutyCycle;

  hypotheses.push(`Chaleur = Pabs × ${(ratioPertes * 100).toFixed(0)} % ` +
    `(circuit ${typeCircuit}${laminage ? ' + laminage' : ''}).`);
  if (dutyParVerin)
    hypotheses.push('Chaleur sommée PAR VÉRIN (Σ Pabs_i × ratio × duty_i × qté_i) : chaque vérin à son débit/pression/taux.');
  if (spec.systeme?.typeCircuit == null)
    hypotheses.push('Topologie de circuit non spécifiée : "ouvert" supposé (le plus courant).');
  if (laminage)
    hypotheses.push('Laminage déclaré -> ratio de pertes porté à 50 % (étranglement dominant).');
  if (holdRatio > 0)
    hypotheses.push(`Maintien ${(holdRatio * 100).toFixed(0)} % du cycle : ` +
      (estVariable ? 'pompe variable -> chaleur réduite.' : 'pompe fixe -> chaleur vers Pabs au limiteur.'));

  const bilan = (dissipationNaturelle != null) ? chaleurGeneree - dissipationNaturelle : null;
  if (bilan != null && bilan > 0) {
    hypotheses.push(`Bilan thermique positif (${bilan.toFixed(2)} kW) -> refroidisseur requis (étape sélection).`);
  }

  // Température d'équilibre SANS refroidisseur (où l'huile se stabilise), avec le modèle de
  // dissipation du moteur : à l'équilibre, chaleur = coeff·surface·(T_eq − T_amb).
  // Cohérent par construction : T_eq ≤ T_max  <=>  bilan ≤ 0.
  const tempEquilibre = (dissipationNaturelle != null && dissipationNaturelle > 0
                          && deltaT != null && fluide.tempAmbiante != null)
    ? fluide.tempAmbiante + chaleurGeneree * deltaT / dissipationNaturelle
    : null;
  if (tempEquilibre != null && fluide.tempHuileMax != null) {
    const ok = tempEquilibre <= fluide.tempHuileMax;
    hypotheses.push(`Sans refroidisseur, l'huile se stabilise à ~${tempEquilibre.toFixed(0)} °C ` +
      `(${ok ? '≤' : '>'} ${fluide.tempHuileMax} °C max) -> ${ok ? 'refroidissement naturel suffisant' : 'refroidisseur nécessaire'}.`);
  }

  return {
    verins,
    systeme: {
      debitTotal,
      debitSimultanes: sommeSimultanes,
      debitAlterneMax: maxAlterne,
      pression: pressionSysteme,
      rendement,
      puissanceHydraulique,
      puissanceAbsorbee,
      pertes,
      vitesseMoteur: spec.systeme?.vitesseMoteur ?? 1450,
      poles: spec.systeme?.poles ?? 4,
      typeUtilisation: spec.systeme?.typeUtilisation ?? 'continu',
      typePompe: spec.systeme?.typePompe ?? null
    },
    reservoir,
    thermique: {
      deltaT,
      typeCircuit,
      laminage,
      ratioPertes,
      chaleurActive,
      chaleurGeneree,
      holdRatio,
      dutyCycle,
      dutyParVerin,
      dissipationNaturelle,
      tempEquilibre,
      bilan
    },
    hypotheses
  };
}

/**
 * selectionnerComposants(besoins, selecteurs) -> composants  [brique 2/2, archi B]
 *
 * Orchestrateur PUR : aucune logique de sélection à l'intérieur. Il appelle des
 * sélecteurs INJECTÉS (tes fonctions prouvées recommendPump / selectWEGMotor /
 * selectHydacCooler) avec les bonnes entrées tirées de besoins, et normalise
 * leurs retours. Zéro réécriture de ta logique catalogue, zéro divergence.
 *
 * Navigateur :
 *   selectionnerComposants(besoins, {
 *     pompe:         recommendPump,
 *     moteur:        selectWEGMotor,
 *     refroidisseur: selectHydacCooler
 *   })
 * Tests : on injecte des sélecteurs bidon (pas besoin des catalogues).
 */
function selectionnerComposants(besoins, selecteurs = {}) {
  const s = besoins.systeme || {};
  const t = besoins.thermique || {};
  const res = { pompe: null, moteur: null, refroidisseur: null, notes: [] };

  if (typeof selecteurs.pompe === 'function') {
    const reco = selecteurs.pompe(s.debitTotal, s.pression, s.vitesseMoteur, s.typeUtilisation, s.typePompe);
    res.pompe = Array.isArray(reco) ? (reco[0] ?? null) : (reco ?? null);
    if (Array.isArray(reco) && reco.length === 0)
      res.notes.push('Aucune pompe ne couvre le débit/pression demandés.');
  }

  if (typeof selecteurs.moteur === 'function') {
    // Le moteur suit la pompe : on le dimensionne sur la puissance absorbée de la
    // pompe choisie (qui dépend de SON rendement), sinon sur l'absorbée système.
    const pAbs = (res.pompe && res.pompe.puissance_absorbee != null)
      ? parseFloat(res.pompe.puissance_absorbee) : s.puissanceAbsorbee;
    const m = selecteurs.moteur(pAbs, s.poles);
    res.moteur = (m && typeof m === 'object' && 'motor' in m) ? m.motor : (m ?? null);
    if (m && m.oversized)
      res.notes.push('Aucun moteur standard assez puissant : surdimensionnement requis.');
  }

  // Refroidisseur seulement si le bilan thermique l'exige
  if (typeof selecteurs.refroidisseur === 'function' && t.bilan != null && t.bilan > 0) {
    res.refroidisseur = selecteurs.refroidisseur(t.bilan, s.debitTotal) ?? null;
  }

  return res;
}

if (typeof module !== 'undefined' && module.exports) {
  module.exports = { calculerBesoins, selectionnerComposants, forceEnDaN, boreDepuisAire };
}
