...
function calculerBesoins(spec) {
  ...
  // Modèle de chaleur — identique à l'appli (type non spécifié => chemin FIXE, conservateur)
  const estVariable = (spec.systeme?.typePompe === 'variable');
  let chaleurGeneree = pertes;                                  // pertes de base
  if (!estVariable) {
    if (holdRatio > 0) chaleurGeneree += puissanceAbsorbee * 0.6 * holdRatio;  // tout au limiteur en maintien
  } else {
    chaleurGeneree *= 0.6;                                       // -40% de pertes vs fixe
    if (holdRatio > 0) chaleurGeneree = Math.max(0.5, chaleurGeneree - chaleurGeneree * 0.3 * holdRatio);
  }
  if (pumpStops && dutyCycle < 1) chaleurGeneree *= dutyCycle;

  if (spec.systeme?.typePompe == null)
    hypotheses.push('Type de pompe non spécifié pour le thermique : chemin FIXE supposé (conservateur).');
  if (holdRatio > 0)
    hypotheses.push(`Maintien de pression modélisé : ${(holdRatio * 100).toFixed(0)} % du cycle.`);
