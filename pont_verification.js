...
function construireSpec() {
  ...
  const neufs = (typeof newCylinders !== 'undefined' ? newCylinders : []).map(c => ({
    ...
    pressureHold: c.pressureHold, holdTime: c.holdTime
  }));

  // Duty cycle (0-1) et arrêt pompe : on lit la même source que calculateThermal de l'appli
  let dutyCycle = 1, pumpStops = false;
  try {
    if (typeof updateDutyCycle === 'function') dutyCycle = updateDutyCycle();
    pumpStops = (typeof dutyMode !== 'undefined' && dutyMode === 'cycle')
      ? !!document.getElementById('pumpStops')?.checked
      : (dutyCycle < 1.0);
  } catch (e) { dutyCycle = 1; pumpStops = false; }

  return {
    systeme: { pression: pressionGlobale, rendement: (val('efficiency') || 90) / 100,
               typePompe: txt('pumpType') === 'fixed' ? 'fixe' : (txt('pumpType') === 'variable' ? 'variable' : null),
               dutyCycle, pumpStops },
